...
if ($get_item_res->num_rows < 1) {
   ...
   // need to check if in sale first with variation id

    // function get price at bottom? becose need color and size first

    //getprice($_GET["item_id"]);
    ...
    $display_block .= "
   <p><strong>Price:</strong> \$".getPrice($itemId)."</p>
...
//  $get_colors_sql = "SELECT item_color FROM store_item_color WHERE item_id = '".$_GET["item_id"]."' ORDER BY item_color";
 // $get_colors_res = $conn->query($get_colors_sql) or die("Couldn't connect");
   ...
   if ($get_colors_res->num_rows > 0) {
       ...
       $color_block .= "<p><strong>Available Colors:</strong><br/>
            <select name=\"sel_item_color\">";
            ...
            if ($get_colors_value_res->num_rows > 0) {
                ...
                while ($colors_value = $get_colors_value_res->fetch_array()) {
                    ...
                    if($color_res != "") {
                        ...
                        $color_block .= "<option value=\"" . $color_res . "\">" . $color_res . "</option>";
                    }
                }
...
$size_id_record = NULL;
   ...
   if ($get_sizes_res->num_rows > 0) {
      ...
      // <select name=\"sel_item_size\">";
       $size_block .= "<p><strong>Available Sizes:</strong><br/>
       <select name=\"sel_item_size\">";
           ...
           if ($get_sizes_value_res->num_rows > 0) {
               ...
               while ($size_value = $get_sizes_value_res->fetch_array()) {
                   ...
                   if ($size_res != "") {
                       ...
                       $sizeFlag=true;
                       $size_block .= "<option value=\"" . $size_res . "\">" . $size_res . "</option>";
                   }
               }
           }
...
<?php echo $display_block; ?>
    ...
    <div>
        ...
        foreach($xml as $feedback){
            ...
            if($feedback->attributes()->id == $_GET["item_id"]) {
                ...
                if($feedback->status == "approved") {
                    ...
                    echo "<td>" . "give ".$feedback->rating . "/10 "."</td>";
                    echo "<td> " ." and said ". $feedback->message . "</td>";
